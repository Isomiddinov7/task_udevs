...
// Login godoc
// @ID login-curier
// @Router /login-curier [POST]
// @Summary login
// @Description login
// @Tags Curier
// @Accept json
// @Produce json
// @Param object body models.CurierAuthRequest true "LoginCurierRequestBody"
// @Success 200 {object} Response{data=string} "Token"
// @Response 400 {object} Response{data=string} "Invalid Argument"
// @Failure 500 {object} Response{data=string} "Server Error"
func (h *Handler) AuthCurier(c *gin.Context) {
	var curier models.CurierAuthRequest

	err := c.ShouldBindJSON(&curier)
	if err != nil {
		handleResponse(c, 400, "http.BadRequest"+err.Error())
		return
	}
	resp, err := h.strg.Curier().Auth(
		c.Request.Context(),
		curier,
	)
	if err != nil {
	...
	access_token, err := utils.CreateToken(config.AccessTokenExpiresIn, resp.CurierId, config.AccessTokenPrivateKey)
	if err != nil {
		handleResponse(c, 500, "InternalServerError: Could not generate token")
		return
	}
	refresh_token, err := utils.CreateToken(config.RefreshTokenExpiresIn, resp.CurierId, config.RefreshTokenPrivateKey)
	if err != nil {
		handleResponse(c, 500, "InternalServerError: Could not generate token")
		return
	}
